    ...
    <Box>
      {
        <Box className="mb-4">
          ...
          {Array.from(sheets.entries()).map(([id, characterMap]) => {
            if (!characterMap) return null;
            return (
              <Box key={id} className="mb-1 p-2 border rounded">
                                 <strong>{((characterMap.get("identity") as Y.Map<string | number>)?.get("name") as string || '')}</strong>                <span className="text-sm text-gray-500 ml-2">({id})</span>
              </Box>
            );
          })}
        </Box>
